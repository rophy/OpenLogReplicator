...
**What gets committed:** Only SQL input files (`sql/inputs/*.sql`) and the test
framework code. Fixture archives (redo logs, schema, golden files) are **not**
committed to git — they are generated by CI and stored as workflow artifacts.

**How fixtures flow:**

1. Developer adds/modifies a `.sql` file in `sql/inputs/`
2. On push to master, CI `sql-tests-*` workflows run `test_e2e.py` against
   live Oracle, generating fixtures in `sql/generated/` and uploading them
   as GitHub Actions artifacts (90-day retention)
3. CI `redo-log-tests` workflow downloads the latest fixture artifacts and
   runs regression tests against them — no Oracle needed

**Local development:** Use `make fixtures` to compress `sql/generated/` into
`fixtures/<scenario>.tar.gz` for local `make test-redo` runs. These local
archives are gitignored (except `example-*` bootstrapping fixtures).
